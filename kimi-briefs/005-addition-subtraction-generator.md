...
Accepted with no changes. Checked sample sets for every level by hand — right answers, good mix of shapes, crossing-10/tens and bonds-to-100 all present. Now used by the new `/child/[id]/maths` screen (shared with times tables via `NumberPractice`).
