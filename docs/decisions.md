...
| 2026-09-26 | Daily goal per child (default 2 sets a day, parent sets 1–5); child home shows today's stars; parent sees last 7 days (green = goal met), sets / minutes / levels passed, accuracy per subject, and "keeps tripping up on" across all subjects |
| 2026-09-26 | Voice: pre-made recordings with Google Cloud TTS **Chirp 3 HD** (en-GB), made once on DQ's PC (`npm run audio`), stored in `public/audio/<hash>.mp3` with a manifest; app plays the recording when there is one, otherwise the device voice. Speaking rate 0.9. Setup in `docs/audio-setup.md`. Phonics pure sounds will still be DQ's own recordings |
